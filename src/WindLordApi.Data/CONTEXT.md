...
- `GetByIdsAsync` for paragliding locations returns only rows where `IsActive && IsMain`, so forecast refresh ignores inactive or secondary locations even if their IDs were selected earlier. `confirmed`
...
## Watchouts
- Renaming or dropping the forecast-selection views requires coordinated changes in `ApplicationDbContext` and the repositories that consume them. `confirmed`
